...
\begin{frame}
\chtitle{Noether定理的证明}
\bch
证明：
\bea
\epsilon \partial_\mu j^\mu  &=& \epsilon \partial_\mu F^\mu -  \frac{\partial\lagr}{\partial (\partial_\mu\phi)} \partial_\mu(\delta\phi) -\partial_\mu\left(\frac{\partial\lagr}{\partial (\partial_\mu\phi)} \right)\delta\phi \newl
&=& \delta \lagr -  \frac{\partial\lagr}{\partial (\partial_\mu\phi)} \delta(\partial_\mu\phi) -\frac{\partial\lagr}{\partial \phi} \delta\phi \newl
&=& 0
\eea
中间我们利用了Euler-Lagrange方程。
\ech
\end{frame}
...
\begin{frame}
  ...
  \bch
  \bitem
  \item[4]{考虑一维空间x和一维时间t构成的时空里的标量场$\phi(x,t)$，若其作用量为
